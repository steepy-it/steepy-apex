...
export function reviewerResponseSchema() {
  return JSON.parse(readFileSync(new URL('../skills/implement/reviewer-response.schema.json', import.meta.url), 'utf8'));
}

export function decodeReviewerResponse(payload, format = 'text', { candidate = false } = {}) {
  const schema = reviewerResponseSchema();
  ...
  if (format === 'json') {
    ...
    if (lines.length !== fields.length || fields.some((field, i) => !lines[i].startsWith(`${field}: `))) throw new Error('expected exactly four ordered fields');
