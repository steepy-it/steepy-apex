...
| `inception-handoff.mjs` | Verifies the exact `inception-handoff: steepy-apex/v1` transfer to init through a `verify`/`project`/`checkpoint`/`prepare`/`finalize` CLI: closed strict-JSON inputs, approval bound to the approved project bytes, a verification list of the recorded code checkpoint plus opaque results documents, the six-field confirmed-inputs record projected to the five Project model v1 keys, and code checkpoints compared for changed, added, and removed paths plus branch/HEAD. A promotion destination that also names a checkpoint inventory path is refused, so init's own writes can never diverge the checkpoint. A promotion table without a `promote` decision for some confirmed surface's standard (`.apex/standards/<name>.md`) is refused too; that checks completeness only, never the text. It binds one local init receipt in the descriptor when init starts and records init completion only after a passing gate, unchanged code, and every promoted text present; it never writes stable documentation, invents approval, or runs the gate. |
| `inception-paths.mjs` | Confines access to the local inception area: only the canonical descriptor, its `*` ignore guard, and exact files of one declared run. It validates paths lexically before normalization, binds the admitted `.apex` mount, rejects globs, empty/dot/parent segments, descendant symlinks, non-ordinary files, and hardlinks through verified descriptors, bounds reads to 1 MiB, and writes per-file atomically with previous-digest comparison and identical-byte no-ops. |
| `inception-state.mjs` | Validates, starts, inspects, and updates the closed v1 inception descriptor through a `start`/`inspect`/`update` CLI: canonical bytes, legal phase/status/approval/init transitions enforced by `assertInceptionTransition`, monotone init whose start binds the handoff and receipt paths (the receipt digest advancing only at completion), completion recorded only through a caller-supplied `verifyInitCompletion` gate and never from its own CLI, verified exact references, guard-before-descriptor start with read-only Git exclusion reporting, and read-only branch/HEAD observation through `observeRepositoryRevision`. It classifies pre-hub state from the descriptor alone and never certifies approval or completion, follows references on inspect, or runs bootstrap, deploy, or Git-mutating commands. |
