...
| `inception-handoff.mjs` | Verifies the exact `inception-handoff: steepy-apex/v1` transfer to init through a `verify`/`project`/`checkpoint`/`prepare`/`finalize` CLI: closed strict-JSON inputs, approval bound to the approved project bytes, a verification list of the recorded code checkpoint plus opaque results documents, the six-field confirmed-inputs record projected to the five Project model v1 keys, and code checkpoints compared for changed, added, and removed paths plus branch/HEAD. A promotion destination that also names a checkpoint inventory path is refused, so init's own writes can never diverge the checkpoint. It binds one local init receipt in the descriptor when init starts and records init completion only after a passing gate, unchanged code, and every promoted text present; it never writes stable documentation, invents approval, or runs the gate. |
