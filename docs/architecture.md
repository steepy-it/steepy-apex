...
| `inception-handoff.mjs` | Verifies the exact `inception-handoff: steepy-apex/v1` transfer to init through a `verify`/`project`/`checkpoint`/`prepare`/`finalize` CLI: closed strict-JSON inputs, approval bound to the approved project bytes, the six-field confirmed-inputs record projected to the five Project model v1 keys, and code checkpoints compared for changed, added, and removed paths plus branch/HEAD. It records the local init receipt and init completion only after a passing gate, unchanged code, and every promoted text present; it never writes stable documentation, invents approval, or runs the gate. |
...
| `inception-state.mjs` | Validates, starts, inspects, and updates the closed v1 inception descriptor through a `start`/`inspect`/`update` CLI: canonical bytes, legal phase/status/approval/init combinations, monotone init, verified exact references, and guard-before-descriptor start with read-only Git exclusion reporting. It classifies pre-hub state from the descriptor alone and never certifies approval or completion, follows references on inspect, or runs bootstrap, deploy, or Git-mutating commands. |
