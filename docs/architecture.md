...
| `inception-state.mjs` | Validates, starts, inspects, and updates the closed v1 inception descriptor through a `start`/`inspect`/`update` CLI: canonical bytes, legal phase/status/approval/init combinations, monotone init whose start binds the handoff and receipt paths (the receipt digest advancing only at completion), completion recorded only through a caller-supplied completion verifier and never from its CLI, verified exact references, guard-before-descriptor start with read-only Git exclusion reporting, and read-only branch/HEAD observation. It classifies pre-hub state from the descriptor alone and never certifies approval or completion, follows references on inspect, or runs bootstrap, deploy, or Git-mutating commands. |
