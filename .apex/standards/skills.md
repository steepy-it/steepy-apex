...
- Work-input access is default-deny for every workflow phase. An active handoff grants only its
  ...
  records that justification, and writes its durable artifact before returning the exact four-field envelopes below.
  Manifests govern eager upstream context, not normal repository source discovery.
  The implement skill/controller validates artifact-first completion and the envelope. The implement handoff writes the
  aggregate diff only as `branch-diff.txt` and records each task-result bullet as
...
- Unplanned discovery requires DONE_WITH_CONCERNS plus exact `discovery:unplanned`, retained through fixes and the compact index; ordinary/unattributed concerns are not discovery. Review uses only criteria/index/diff for both insight attribution and regular bump proposals; insufficient evidence goes to the human, not upstream reads. Discovery's explorer completion is self-contained in its Output Format heading, never borrowed through numeric schema anchors.
